...
\begin{itemize}[leftmargin=2em]
  \item \txt{backend/analytics/indicateurs.py} : matrice complète
        ...
        instruction $\times$ chef de ménage sans éducation ($r=0{,}96$),
        ruralité $\times$ dépendance jeunes ($r=0{,}91$).
  \item \txt{backend/analytics/graph.py} : réseau de similarité entre
        wilayas ($|r|>0{,}85$, top-2 voisins) et graphe de corrélations
        entre indicateurs ($|r|>0{,}75$). Page \txt{/reseau}.
...
\begin{meth}{Séparation complète et pseudo-R²}
Le facteur « chef de ménage sans éducation » est \emph{parfaitement
prédictif} (tout enfant concerné est hors école) : la vraisemblance n'a pas
de maximum fini, le coefficient n'est pas estimable. Le code le détecte et le
traite par les règles d'association (confiance 100\,\%, lift 3{,}02, cf.\
§~\ref{sec:apriori}).
\[
...
associées au hors-école, sans hypothèse a priori : milieu rural, pauvreté,
absence d'éducation du chef de ménage, région $\to$ hors école.
...
affiche les règles triées par lift, les règles sans la variable « chef de
ménage » (\emph{sans\_cm}) et les règles par région (\emph{par\_region}).
Résultat : la variable « chef de ménage sans éducation » produit une règle
à confiance 100\,\% et lift 3{,}02 -- c'est le facteur individuel le plus
fort, là où la régression logistique ne pouvait pas l'estimer.
\end{where}
